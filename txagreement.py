...
AGREEMENT_TEMPLATE = os.path.join(TEX_PATH, "agreement.tex")
...
def render_agreement(data, filename=None, verbose=False):
    if data.get("total_words", False) == True:
        data["total_words"] = amount_words(str(data["total"]))
    return render_tex(AGREEMENT_TEMPLATE, data, filename=filename, verbose=verbose)
